...
import { calculateSchool } from '../src/calc';
import { buildStatement, DEFAULT_CATEGORY_GROUPS } from '../src/statement';

test('statement: legacy assertions, ported 1:1', async () => {
  ...
  const { consolidate } = await import('../src/calc');
