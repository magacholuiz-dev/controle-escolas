// Business-health metrics built on top of a report already produced by calc. Every ratio returns
// `null` instead of NaN/Infinity when it can't be computed (e.g. no children, no revenue) — the
// front shows "—" for null, never a broken number.
import type { Report } from './types';

// Categories treated as variable (they grow with the number of children), matching the same
// budgeted+oneOff amount the income statement uses.
...
export interface Metrics {
  ...
  variableCostPerChild: number | null;
  payrollOverRevenue: number | null;
  breakEven: number | null;
  margin: number;
}

export function buildMetrics({ report, activeChildren }: { report: Pick<Report, 'totals' | 'categories' | 'margin'>; activeChildren: number }): Metrics {
...
export function betterSchool(values: { id: string; value: number | null }[], direction: 'lower' | 'higher' = 'higher'): string | null {
