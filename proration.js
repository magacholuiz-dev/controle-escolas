...
// Splits an amount between schools. Returns [{ school_id, pct, amount }] with the cents reconciled to the total.
//  - equal:    even shares
...
export function prorate(total, schools, mode, percentages = {}) {
  ...
  [...weights.keys()].sort((a, b) => weights[b] - weights[a]).forEach((i) => { if (remainder > 0) { shares[i]++; remainder--; } });
  return schools.map((s, i) => ({ school_id: s.id, pct: Math.round((weights[i] / sum) * 10000) / 100, amount: shares[i] / 100 }));
}
