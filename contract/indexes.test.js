...
test('the Nest schemas create the same collections and indexes as the legacy app', { skip: !nestBuilt && 'apps/api is not built' }, async () => {
  ...
  try {
    ...
    // Index builds run in the background: poll (up to 15 s) until both sides agree instead of guessing a delay.
    let [a, b] = [await snapshot(legacy.uri), await snapshot(nest.uri)];
    for (let i = 0; i < 30 && JSON.stringify(a) !== JSON.stringify(b); i++) {
      await new Promise((r) => setTimeout(r, 500));
      [a, b] = [await snapshot(legacy.uri), await snapshot(nest.uri)];
    }
